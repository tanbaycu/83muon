...
const CardViewer = () => {
  ...
  const handleOpenCard = async () => {
    ...
    // Chuẩn hóa ID: đệm số 0 nếu là số nguyên (01, 02, ... 09, 10, ...)
    let queryStt = stt.trim();
    if (/^\d+$/.test(queryStt)) {
      const num = parseInt(queryStt, 10);
      queryStt = num.toString().padStart(2, '0');
    }
    ...
    try {
      ...
      if (docSnap.exists()) {
        // Nếu có dữ liệu trong Firestore, lấy ảnh của tất cả entries để làm film strip background
        getDocs(collection(db, 'girls')).then((snapshot) => {
          const imgs = [];
          snapshot.forEach((d) => {
            if (d.data().imageUrl && d.id !== queryStt) {
              imgs.push(d.data().imageUrl);
            }
          });
          imgs.sort(() => 0.5 - Math.random());
          setAllGirlsImages(imgs);
        }).catch(() => setAllGirlsImages([]));
        ...
        alert('No data found for this identifier.');
        setIsInitiating(false);
      }
    } catch (err) {
      console.error(err);
